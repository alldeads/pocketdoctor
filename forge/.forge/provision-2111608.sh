...
/usr/bin/php7.2 /usr/local/bin/composer install --no-interaction --prefer-dist --optimize-autoloader

if [ -f artisan ]
then
    /usr/bin/php7.2 artisan migrate --force
    /usr/bin/php7.2 artisan view:clear
    /usr/bin/php7.2 artisan queue:restart
fi
